...
| PROTOCOL-001 | Benchmark testing should define setup, order, stop rules, recording fields, and retest notes before logs are interpreted | Equipment access, benchmark path, component protocol | B/D/E | TEST-6MWT-ATS, TEST-FIELD-ADULT, TEST-FIELD-SAFETY, LOG-001 | allowed_ui | Protocol discipline improves repeatability; it does not validate benchmark cutoffs |
| LOG-001 | Benchmark sessions should store raw component results, RPE, equipment, substitutions, protocol version, and notes before any scoring layer | Benchmark component fields | D/E | WODIS-inspired open-source scan, TEST-FIELD-ADULT, TEST-FIELD-SAFETY | allowed_ui | Exportable logs support future validation; not a score or prediction |
