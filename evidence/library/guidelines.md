# Guidelines Library

## PA-WHO-2020

- Title: WHO guidelines on physical activity and sedentary behaviour
- Organization: World Health Organization
- Year: 2020
- URL: https://www.who.int/publications/i/item/9789240015128
- NCBI Bookshelf: https://www.ncbi.nlm.nih.gov/books/NBK566046/
- PubMed: https://pubmed.ncbi.nlm.nih.gov/33239350/
- Evidence tier: A
- SportRx use:
  - adult aerobic activity reference range,
  - muscle-strengthening reference,
  - "some activity is better than none" progression language.
- Limit:
  - public-health recommendation, not HYROX readiness or performance prediction.

## PA-CDC-ADULT

- Title: Adult Activity: An Overview
- Organization: CDC
- URL: https://www.cdc.gov/physical-activity-basics/guidelines/adults.html
- Evidence tier: A
- SportRx use:
  - simple user-facing explanation of weekly adult activity target,
  - reinforcement of 150 minutes/week and 2 strengthening days/week.
- Limit:
  - U.S. public-health summary, not individualized sport prescription.

## PA-ACSM-GETP12

- Title: ACSM's Guidelines for Exercise Testing and Prescription, 12th edition
- Organization: American College of Sports Medicine
- URL: https://acsm.org/education-resources/books/guidelines-exercise-testing-prescription/
- Evidence tier: A
- SportRx use:
  - FITT-VP structure,
  - exercise testing and prescription framing,
  - applied settings and resistance training context.
- Limit:
  - full text is a book; do not copy proprietary content into SportRx.

## PA-ACSM-CDC

- Title: ACSM Physical Activity Guidelines
- Organization: American College of Sports Medicine
- URL: https://acsm.org/education-resources/trending-topics-resources/physical-activity-guidelines/
- Evidence tier: A
- SportRx use:
  - convergent support for general activity guidance.
- Limit:
  - general guideline summary, not event-specific.

## PA-AHA

- Title: American Heart Association Recommendations for Physical Activity in Adults and Kids
- Organization: American Heart Association
- URL: https://www.heart.org/en/healthy-living/exercise-and-physical-activity/fitness-basics/aha-recs-for-physical-activity-in-adults
- Evidence tier: A
- SportRx use:
  - convergent public-health guidance for aerobic and strengthening activity.
- Limit:
  - cardiovascular-health framing, not HYROX preparation validation.

## PA-CHINA-2021

- Title: 中国人群身体活动指南（2021）
- Organization: 国家卫生健康委疾病预防控制局指导；中国疾病预防控制中心和国家体育总局体育科学研究所牵头
- Public interpretation: https://www.nhc.gov.cn/wjw/ftsp/202112/44e8325ad5934eb0b42b2987e3148315.shtml
- Evidence tier: A
- SportRX use:
  - Chinese-language public-health activity-guidance context for adults,
  - local source provenance for Chinese-facing explanation.
- Limit:
  - the full guideline is copyrighted and not saved in this repository;
  - it does not validate SportRX Benchmark cutoffs, athlete norms, or HYROX outcomes.
