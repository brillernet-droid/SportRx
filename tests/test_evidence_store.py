...
def test_structured_evidence_records_validate_and_cover_current_rules():
    ...
    assert validation["counts"] == {"sources": 56, "claims": 48, "rules": 17, "protocols": 9}
...
def test_internal_index_search_and_rule_trace_return_sources_and_limits(tmp_path):
    ...
    assert index["document_count"] == 130
    assert db_path.exists()
    with sqlite3.connect(db_path) as connection:
        assert connection.execute("SELECT count(*) FROM evidence_fts").fetchone()[0] == 130
