...
def test_release_package_manifest_is_public_ready():
    ...
    assert "sportrx/hypertrophy_atlas.py" in manifest["included_files"]
    assert "data/exercises/hypertrophy_atlas.json" in manifest["included_files"]
    assert "evidence/knowledge/packs/hypertrophy_movement_v1.json" in manifest["included_files"]
    ...
    assert "evidence/knowledge/packs/training_science_v1.json" in manifest["included_files"]
    assert "evidence/records/packs/training_science_sources.json" in manifest["included_files"]
    assert "evidence/records/packs/training_science_claims.json" in manifest["included_files"]
    assert "evidence/training_science/manifest.json" in manifest["included_files"]
    assert "scripts/fetch_open_access_training_science_reviews.py" in manifest["included_files"]
...
def test_write_release_package_excludes_internal_files(tmp_path):
    ...
    assert "sportrx/hypertrophy_atlas.py" in names
    assert "data/exercises/hypertrophy_atlas.json" in names
    assert "evidence/knowledge/packs/hypertrophy_movement_v1.json" in names
